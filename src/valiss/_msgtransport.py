# pyright: strict
"""Transport-agnostic core of the message-token (proof-of-origin) transports:
the emitter-creds minter, the trust-anchor binding, and the receiver's
verify-with-chain-negotiation state machine. Shared by ``httpsig`` (HTTP) and
``grpcsig`` (gRPC); each supplies its own audience, payload bytes, and wire
plumbing. Pure logic — no httpx, grpc, or framework dependency.
"""
...
def build_verifier(
    ...
    if (operator_pub_key is None) == (keyring is None):
        raise ValissError(
            "valiss: message-token middleware requires exactly one of operator_pub_key or keyring"
        )
...
class Reject:
    """A rejection: the message, and whether to attach the ``valiss-chain:
    required`` signal asking the client to retransmit its chain."""
...
def authenticate_message(
    ...
    *,
    ...
    receiving side of chain negotiation. Detached chain material outranks the
